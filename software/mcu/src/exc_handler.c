...
void __attribute__ ((naked, aligned(8))) _hardfault_isr()
{
    __asm__ volatile
    (
        " tst lr, #4                                                \n" // On exception entry the CPU pushes PSR, PC, LR, R12, R3, R2, R1 and R0 onto the stack in this order, so SP points to stacked R0
        " ite eq                                                    \n" // Bit 2 of LR indicates if PSP or MSP is in use
        " mrseq r0, msp                                             \n" // If 0, MSP is in use, Mode MSP to R0
        " mrsne r0, psp                                             \n" // If 1, PSP is in use, Move PSP to R0
        " ldr r1, [r0, #24]                                         \n" // Move the PC where the fault happened to R1 (not used)
        " ldr r2, hardfault_trace_stack_addr                        \n" // Load the C routine address into R2
        " bx r2                                                     \n" // Branch to R2
